...
#define Demo_GDataXMLNODE_DEFINE_GLOBALS 1
#import "Demo_GDataXMLParser.h"

@class NSArray, NSDictionary, NSError, NSString, NSURL;
@class Demo_GDataXMLElement, Demo_GDataXMLDocument;


static const int kDemo_GDataXMLParseOptions = (XML_PARSE_NOCDATA | XML_PARSE_NOBLANKS);
...
// Demo_GDataXMLNode does not require pulling in all of GData.
...
static NSString *GDataFakeQNameForURIAndName(NSString *theURI, NSString *name) {
  ...
  NSString *localName = [Demo_GDataXMLNode localNameForName:name];
...
@interface Demo_GDataXMLNode (PrivateMethods)
...
- (Demo_GDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode;
...
+ (Demo_GDataXMLElement *)elementWithName:(NSString *)name {
...
+ (Demo_GDataXMLElement *)elementWithName:(NSString *)name stringValue:(NSString *)value {
...
+ (Demo_GDataXMLElement *)elementWithName:(NSString *)name URI:(NSString *)theURI {
...
+ (id)nodeConsumingXMLNode:(xmlNodePtr)theXMLNode {
  ...
  if (theXMLNode->type == XML_ELEMENT_NODE) {
    theClass = [Demo_GDataXMLElement class];
  } else {
    theClass = [Demo_GDataXMLNode class];
  }
...
#if DEBUG
  NSCAssert(chars != NULL, @"Demo_GDataXMLNode sees an unexpected empty string");
#endif
...
- (NSArray *)children {
  ...
  if (xmlNode_ != NULL) {
    ...
    while (currChild != NULL) {
      Demo_GDataXMLNode *node = [Demo_GDataXMLNode nodeBorrowingXMLNode:currChild];
...
- (Demo_GDataXMLNode *)childAtIndex:(unsigned)index {
...
- (Demo_GDataXMLNodeKind)kind {
  if (xmlNode_ != NULL) {
    xmlElementType nodeType = xmlNode_->type;
    switch (nodeType) {
      case XML_ELEMENT_NODE:         return Demo_GDataXMLElementKind;
      case XML_ATTRIBUTE_NODE:       return Demo_GDataXMLAttributeKind;
      case XML_TEXT_NODE:            return Demo_GDataXMLTextKind;
      case XML_CDATA_SECTION_NODE:   return Demo_GDataXMLTextKind;
      case XML_ENTITY_REF_NODE:      return Demo_GDataXMLEntityDeclarationKind;
      case XML_ENTITY_NODE:          return Demo_GDataXMLEntityDeclarationKind;
      case XML_PI_NODE:              return Demo_GDataXMLProcessingInstructionKind;
      case XML_COMMENT_NODE:         return Demo_GDataXMLCommentKind;
      case XML_DOCUMENT_NODE:        return Demo_GDataXMLDocumentKind;
      case XML_DOCUMENT_TYPE_NODE:   return Demo_GDataXMLDocumentKind;
      case XML_DOCUMENT_FRAG_NODE:   return Demo_GDataXMLDocumentKind;
      case XML_NOTATION_NODE:        return Demo_GDataXMLNotationDeclarationKind;
      case XML_HTML_DOCUMENT_NODE:   return Demo_GDataXMLDocumentKind;
      case XML_DTD_NODE:             return Demo_GDataXMLDTDKind;
      case XML_ELEMENT_DECL:         return Demo_GDataXMLElementDeclarationKind;
      case XML_ATTRIBUTE_DECL:       return Demo_GDataXMLAttributeDeclarationKind;
      case XML_ENTITY_DECL:          return Demo_GDataXMLEntityDeclarationKind;
      case XML_NAMESPACE_DECL:       return Demo_GDataXMLNamespaceKind;
      case XML_XINCLUDE_START:       return Demo_GDataXMLProcessingInstructionKind;
      case XML_XINCLUDE_END:         return Demo_GDataXMLProcessingInstructionKind;
      case XML_DOCB_DOCUMENT_NODE:   return Demo_GDataXMLDocumentKind;
    }
  }
  return Demo_GDataXMLInvalidKind;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
  ...
  // are created from Demo_GDataXMLElement's initWithXMLString there may not be
  // a document. (We may later decide that we want to stuff the doc used
  // there into a Demo_GDataXMLDocument and retain it, but we don't do that now.)
  //
    ...
    if (xpathCtx) {
      ...
      if (namespaces) {
        ...
        for (NSString *prefix in namespaces) {
          ...
          if (result != 0) {
#if DEBUG
            NSCAssert1(result == 0, @"Demo_GDataXMLNode XPath namespace %@ issue",
                      prefix);
#endif
          }
        }
        ...
        if (nsNodePtr != NULL) {
          for (xmlNsPtr nsPtr = nsNodePtr->ns; nsPtr != NULL; nsPtr = nsPtr->next) {
            ...
            if (prefix == NULL) {
              prefix = (xmlChar*) kDemo_GDataXMLXPathDefaultNamespacePrefix;
            }

            int result = xmlXPathRegisterNs(xpathCtx, prefix, nsPtr->href);
            if (result != 0) {
#if DEBUG
              NSCAssert1(result == 0, @"Demo_GDataXMLNode XPath namespace %s issue",
                        prefix);
#endif
            }
          }
      ...
      if (xpathObj) {
        ...
        if (nodeSet) {
          ...
          for (int index = 0; index < nodeSet->nodeNr; index++) {
            ...
            Demo_GDataXMLNode *node = [Demo_GDataXMLNode nodeBorrowingXMLNode:currNode];
            if (node) {
              [array addObject:node];
            }
          }
  ...
  if (array == nil && error != nil) {
    *error = [NSError errorWithDomain:@"com.google.Demo_GDataXML"
                                 code:errorCode
                             userInfo:errorInfo];
  }
...
- (BOOL)isEqual:(Demo_GDataXMLNode *)other {
  if (self == other) return YES;
  if (![other isKindOfClass:[Demo_GDataXMLNode class]]) return NO;
...
- (NSUInteger)hash {
  return (NSUInteger) (void *) [Demo_GDataXMLNode class];
}
...
@implementation Demo_GDataXMLElement
  ...
  if (xmlNode_ != NULL && xmlNode_->nsDef != NULL) {
    ...
    while (currNS != NULL) {
      ...
      if (!xmlStrEqual(currNS->prefix, (const xmlChar *) "xml")) {
        Demo_GDataXMLNode *node = [Demo_GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) currNS];
...
- (void)setNamespaces:(NSArray *)namespaces {
  ...
  if (xmlNode_ != NULL) {
    ...
    Demo_GDataXMLNode *namespaceNode;
...
- (void)addNamespace:(Demo_GDataXMLNode *)aNamespace {
...
- (void)addChild:(Demo_GDataXMLNode *)child {
  if ([child kind] == Demo_GDataXMLAttributeKind) {
...
- (void)removeChild:(Demo_GDataXMLNode *)child {
...
- (NSArray *)elementsForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    for (Demo_GDataXMLNode *child in children) {
...
- (NSArray *)attributes {
  ...
  if (xmlNode_ != NULL && xmlNode_->properties != NULL) {
    ...
    while (prop != NULL) {
      ...
      Demo_GDataXMLNode *node = [Demo_GDataXMLNode nodeBorrowingXMLNode:(xmlNodePtr) prop];
      if (array == nil) {
        array = [NSMutableArray arrayWithObject:node];
      } else {
        [array addObject:node];
...
- (void)addAttribute:(Demo_GDataXMLNode *)attribute {
...
- (Demo_GDataXMLNode *)attributeForXMLNode:(xmlAttrPtr)theXMLNode {
  // search the cached attributes list for the Demo_GDataXMLNode with
  // the underlying xmlAttrPtr
  NSArray *attributes = [self attributes];

  for (Demo_GDataXMLNode *attr in attributes) {
...
- (Demo_GDataXMLNode *)attributeForName:(NSString *)name {
  ...
  if (xmlNode_ != NULL) {
    ...
    if (attrPtr) {
      Demo_GDataXMLNode *attr = [self attributeForXMLNode:attrPtr];
      return attr;
    }
  }
  return nil;
...
- (Demo_GDataXMLNode *)attributeForLocalName:(NSString *)localName
...
@interface Demo_GDataXMLDocument (PrivateMethods)
- (void)addStringsCacheToDoc;
@end

@implementation Demo_GDataXMLDocument

- (id)initWithXMLString:(NSString *)str options:(unsigned int)mask error:(NSError **)error {

  NSData *data = [str dataUsingEncoding:NSUTF8StringEncoding];
  Demo_GDataXMLDocument *doc = [self initWithData:data options:mask error:error];
  return doc;
}
...
- (id)initWithData:(NSData *)data options:(unsigned int)mask error:(NSError **)error {
  ...
  if (self) {
    ...
    xmlDoc_ = xmlReadMemory((const char*)[data bytes], (int)[data length], baseURL, encoding,
                            kDemo_GDataXMLParseOptions); // TODO(grobbins) map option values
    if (xmlDoc_ == NULL) {
      if (error) {
       *error = [NSError errorWithDomain:@"com.google.Demo_GDataXML"
                                    code:-1
...
- (id)initWithRootElement:(Demo_GDataXMLElement *)element {
...
- (void)addStringsCacheToDoc {
  ...
  NSCAssert(xmlDoc_ != NULL && xmlDoc_->_private == NULL,
            @"Demo_GDataXMLDocument cache creation problem");
#endif
...
- (Demo_GDataXMLElement *)rootElement {
  Demo_GDataXMLElement *element = nil;

  if (xmlDoc_ != NULL) {
    xmlNodePtr rootNode = xmlDocGetRootElement(xmlDoc_);
    if (rootNode) {
      element = [Demo_GDataXMLElement nodeBorrowingXMLNode:rootNode];
    }
  }
  return element;
}
...
- (NSArray *)nodesForXPath:(NSString *)xpath
                namespaces:(NSDictionary *)namespaces
                     error:(NSError **)error {
  if (xmlDoc_ != NULL) {
    Demo_GDataXMLNode *docNode = [Demo_GDataXMLElement nodeBorrowingXMLNode:(xmlNodePtr)xmlDoc_];
